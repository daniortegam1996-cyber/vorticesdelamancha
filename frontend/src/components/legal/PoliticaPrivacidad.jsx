      ...
      <div className="prose prose-lg max-w-none text-gray-700">
        ...
        <section className="mb-8">
          ...
          <ul className="list-disc pl-6 mt-4 space-y-2">
            <li><strong>Responsable:</strong> {DATOS_ASOCIACION.nombreCompleto}</li>
            <li><strong>CIF:</strong> {DATOS_ASOCIACION.cif}</li>
            <li><strong>Dirección:</strong> {DATOS_ASOCIACION.direccion}, {DATOS_ASOCIACION.codigoPostal} {DATOS_ASOCIACION.ciudad} ({DATOS_ASOCIACION.provincia})</li>
            <li><strong>Email de contacto:</strong> {DATOS_ASOCIACION.emailPrivacidad}</li>
          </ul>
        </section>
        ...
        <section className="mb-8">
          ...
          <p className="mt-4">Para ejercer estos derechos, puede enviar una solicitud a <a href={`mailto:${DATOS_ASOCIACION.emailPrivacidad}`} className="text-[#0074D9] hover:underline">{DATOS_ASOCIACION.emailPrivacidad}</a> adjuntando copia de su DNI o documento identificativo.</p>
          <p className="mt-4">Asimismo, tiene derecho a presentar una reclamación ante la <strong>Agencia Española de Protección de Datos</strong> (<a href="https://www.aepd.es" target="_blank" rel="noopener noreferrer" className="text-[#0074D9] hover:underline">www.aepd.es</a>) si considera que sus derechos no han sido debidamente atendidos.</p>
        </section>

        <section className="mb-8">
          <h2 className="text-2xl font-bold text-[#001f3f] mb-4">8. Medidas de Seguridad</h2>
          <p>{DATOS_ASOCIACION.nombre} ha adoptado las medidas técnicas y organizativas necesarias para garantizar la seguridad de los datos personales y evitar su alteración, pérdida, tratamiento o acceso no autorizado, habida cuenta del estado de la tecnología, la naturaleza de los datos almacenados y los riesgos a que están expuestos.</p>
        </section>

        <section className="mb-8">
          <h2 className="text-2xl font-bold text-[#001f3f] mb-4">9. Modificaciones de la Política de Privacidad</h2>
          <p>{DATOS_ASOCIACION.nombre} se reserva el derecho a modificar la presente Política de Privacidad para adaptarla a novedades legislativas o jurisprudenciales. En dichos supuestos, se anunciarán los cambios introducidos con razonable antelación a su puesta en práctica.</p>
        </section>

        <section className="mt-12 p-6 bg-gray-100 rounded-xl">
          <p className="text-sm text-gray-600">
            <strong>Contacto para cuestiones de privacidad:</strong><br />
            {DATOS_ASOCIACION.responsablePrivacidad}<br />
            Email: <a href={`mailto:${DATOS_ASOCIACION.emailPrivacidad}`} className="text-[#0074D9] hover:underline">{DATOS_ASOCIACION.emailPrivacidad}</a>
          </p>
        </section>
      </div>
    </LegalLayout>
